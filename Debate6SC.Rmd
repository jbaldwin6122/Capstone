...
url <- ("https://www.presidency.ucsb.edu/documents/republican-candidates-debate-north-charleston-south-carolina")
debate6 <- read_html(url)
participants6 <- "BUSH|CARSON|CHRISTIE|CRUZ|KASICH|RUBIO|TRUMP|BARTIROMO|CAVUTO"
participantsRemove6 <- "BUSH:|CARSON:|CHRISTIE:|CRUZ:|KASICH:|RUBIO:|TRUMP:|BARTIROMO:|CAVUTO:"
```

## Sentiment Analysis
```{r, include = FALSE}
debate6DF <- scrape_transcript(debate6, participants6, participantsRemove6)
```
...
lsdDf <- dfm(debate6DF$text, dictionary = data_dictionary_LSD2015)

newLsdDf <- convert(lsdDf, to = "data.frame")

newLsdDf$doc_id <- debate6DF$speaker
...
barDFNRC %>%
  filter(speaker != "BARTIROMO" , speaker != "CAVUTO") %>%
...
dumbellDFLSD %>%
  ...
  geom_text(data = filter(dumbellDFLSD, speaker == "BUSH"), 
            aes(x = 3.18, y = speaker, label = "Negative"), 
            color = red, size = 2.5, fontface = "bold", vjust = -1)+
  geom_text(data = filter(dumbellDFLSD, speaker == "BUSH"), 
            aes(x = 4.62, y = speaker, label = "Positive"), 
  ...
  geom_text(data = filter(dumbellDFLSD, speaker == "BUSH"), aes(x = 6.75, y = speaker, label = "Difference"), color = "black", size = 2.5, vjust = -.78, fontface = "bold") + labs(x = "emotive language as % of total words", y = "speaker", title = "Sentiment differences in first 2016 GOP Primary Debate (Lexicoder Sentiment Dictionary)", subtitle = "Carson exhibits the greatest preference for positive diction" ) + theme_bw()+
theme(
 # panel.grid.major=element_blank(),
  panel.grid.minor=element_blank(),
  panel.border=element_blank(),
