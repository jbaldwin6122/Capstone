...
url <- ("https://www.presidency.ucsb.edu/documents/republican-candidates-debate-miami-florida")
debate12 <- read_html(url)
participants12 <- "CRUZ|KASICH|RUBIO|TRUMP|TAPPER|BASH|HEWITT|DINAN"
participantsRemove12 <- "CRUZ:|KASICH:|RUBIO:|TRUMP:|TAPPER:|BASH:|HEWITT:|DINAN:"
```

## Sentiment Analysis
```{r, include = FALSE}
debate12DF <- scrape_transcript(debate12, participants12, participantsRemove12)
```
...
lsdDf <- dfm(debate12DF$text, dictionary = data_dictionary_LSD2015)

newLsdDf <- convert(lsdDf, to = "data.frame")

newLsdDf$doc_id <- debate12DF$speaker
...
dumbellDFLSD %>%
  ...
  geom_text(data = filter(dumbellDFLSD, speaker == "KASICH"), 
            aes(x = 2.15, y = speaker, label = "Negative"), 
            color = red, size = 2.5, fontface = "bold", vjust = -1)+
  geom_text(data = filter(dumbellDFLSD, speaker == "KASICH"), 
            aes(x = 4.25, y = speaker, label = "Positive"), 
  ...
  geom_text(data = filter(dumbellDFLSD, speaker == "KASICH"), aes(x = 6.75, y = speaker, label = "Difference"), color = "black", size = 2.5, vjust = -.78, fontface = "bold") + labs(x = "emotive language as % of total words", y = "speaker", title = "Sentiment differences in first 2016 GOP Primary Debate (Lexicoder Sentiment Dictionary)", subtitle = "Kasich exhibits the greatest preference for positive diction" ) + theme_bw()+
theme(
 # panel.grid.major=element_blank(),
  panel.grid.minor=element_blank(),
  panel.border=element_blank(),
