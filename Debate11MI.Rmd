...
url <- ("https://www.presidency.ucsb.edu/documents/republican-candidates-debate-detroit-michigan")
debate11 <- read_html(url)
participants11 <- "CRUZ|KASICH|RUBIO|TRUMP|BAIER|KELLY|WALLACE"
participantsRemove11 <- "CRUZ:|KASICH:|RUBIO:|TRUMP:|BAIER:|KELLY:|WALLACE:"
```

## Sentiment Analysis
```{r, include = FALSE}
debate11DF <- scrape_transcript(debate11, participants11, participantsRemove11)
```
...
lsdDf <- dfm(debate11DF$text, dictionary = data_dictionary_LSD2015)

newLsdDf <- convert(lsdDf, to = "data.frame")

newLsdDf$doc_id <- debate11DF$speaker
...
dumbellDFLSD %>%
  ...
  geom_text(data = filter(dumbellDFLSD, speaker == "KASICH"), 
            aes(x = 1.69, y = speaker, label = "Negative"), 
            color = red, size = 2.5, fontface = "bold", vjust = -1)+
  geom_text(data = filter(dumbellDFLSD, speaker == "KASICH"), 
            aes(x = 4.26, y = speaker, label = "Positive"), 
