...
url <- ("https://www.presidency.ucsb.edu/documents/republican-candidates-debate-simi-valley-california-0")
debate2 <- read_html(url)
participants2 <- "BUSH|CARSON|CHRISTIE|CRUZ|HUCKABEE|KASICH|PAUL|RUBIO|TRUMP|WALKER|BAIER|KELLY|WALLACE|FIORINA"
participantsRemove2 <- "BUSH:|CARSON:|CHRISTIE:|CRUZ:|HUCKABEE:|KASICH:|PAUL:|RUBIO:|TRUMP:|WALKER:|BAIER:|KELLY:|WALLACE:|FIORINA:"
```

## Sentiment Analysis
```{r, include = FALSE}
debate2DF <- scrape_transcript(debate2, participants2, participantsRemove2)
```
...
lsdDf <- dfm(debate2DF$text, dictionary = data_dictionary_LSD2015)

newLsdDf <- convert(lsdDf, to = "data.frame")

newLsdDf$doc_id <- debate2DF$speaker
...
dumbellDFLSD %>%
  ...
  geom_text(data = filter(dumbellDFLSD, speaker == "BUSH"), 
            aes(x = 2.35, y = speaker, label = "Negative"), 
            color = red, size = 2.5, fontface = "bold", vjust = -1)+
  geom_text(data = filter(dumbellDFLSD, speaker == "BUSH"), 
            aes(x = 5.59, y = speaker, label = "Positive"), 
  ...
  geom_text(data = filter(dumbellDFLSD, speaker == "BUSH"), aes(x = 6.75, y = speaker, label = "Difference"), color = "black", size = 2.5, vjust = -.78, fontface = "bold") + labs(x = "emotive language as % of total words", y = "speaker", title = "Sentiment differences in first 2016 GOP Primary Debate (Lexicoder Sentiment Dictionary)", subtitle = "Kasich exhibits the greatest preference for positive diction" ) + theme_bw()+
theme(
 # panel.grid.major=element_blank(),
  panel.grid.minor=element_blank(),
  panel.border=element_blank(),
...
debate2DF %>% 
  ...
  geom_col() + coord_flip() + labs(title = "Who Talked The Most?", subtitle = "Trump used the most words", x = "speaker", y = "total words") + theme_bw()+
theme(
 # panel.grid.major=element_blank(),
  panel.grid.minor=element_blank(),
  panel.border=element_blank(),
