...
url <- ("https://www.presidency.ucsb.edu/documents/republican-candidates-debate-las-vegas-nevada-0")
debate5 <- read_html(url)
participants5 <- "BUSH|CARSON|CHRISTIE|CRUZ|KASICH|PAUL|RUBIO|TRUMP|BLITZER|BASH|HEWITT|FIORINA"
participantsRemove5 <- "BUSH:|CARSON:|CHRISTIE:|CRUZ:|KASICH:|PAUL:|RUBIO:|TRUMP:|BASH:|HEWITT:|BLITZER:|FIORINA:"
```

## Sentiment Analysis
```{r, include = FALSE}
debate5DF <- scrape_transcript(debate5, participants5, participantsRemove5)
```
...
lsdDf <- dfm(debate5DF$text, dictionary = data_dictionary_LSD2015)

newLsdDf <- convert(lsdDf, to = "data.frame")

newLsdDf$doc_id <- debate5DF$speaker
...
barDFNRC %>%
  filter(speaker != "HEWITT" , speaker != "BASH", speaker != "BLITZER") %>%
...
dumbellDFLSD %>%
  ...
  geom_text(data = filter(dumbellDFLSD, speaker == "CARSON"), 
            aes(x = 3.42, y = speaker, label = "Negative"), 
            color = red, size = 2.5, fontface = "bold", vjust = -1)+
  geom_text(data = filter(dumbellDFLSD, speaker == "CARSON"), 
            aes(x = 5.51, y = speaker, label = "Positive"), 
  ...
  geom_rect(aes(xmin = 6.5, xmax = 7.10, ymin = -Inf, ymax = Inf), fill = "grey") +
  geom_text(aes(label = paste0(round(difference, digits = 2), "%"), y = speaker, x = 6.8), size = 2.5)  +
