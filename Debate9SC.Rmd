...
url <- ("https://www.presidency.ucsb.edu/documents/republican-candidates-debate-greenville-south-carolina")
debate9 <- read_html(url)
participants9 <- "BUSH|CARSON|CRUZ|KASICH|RUBIO|TRUMP|DICKERSON"
participantsRemove9 <- "BUSH:|CARSON:|CRUZ:|KASICH:|RUBIO:|TRUMP:|DICKERSON:"
```

## Sentiment Analysis
```{r, include = FALSE}
debate9DF <- scrape_transcript(debate9, participants9, participantsRemove9)
```
...
lsdDf <- dfm(debate9DF$text, dictionary = data_dictionary_LSD2015)

newLsdDf <- convert(lsdDf, to = "data.frame")

newLsdDf$doc_id <- debate9DF$speaker
...
dumbellDFLSD %>%
  ...
  geom_text(data = filter(dumbellDFLSD, speaker == "CARSON"), 
            aes(x = 2.64, y = speaker, label = "Negative"), 
            color = red, size = 2.5, fontface = "bold", vjust = -1)+
  geom_text(data = filter(dumbellDFLSD, speaker == "CARSON"), 
            aes(x = 4.53, y = speaker, label = "Positive"), 
