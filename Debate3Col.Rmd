...
url <- ("https://www.presidency.ucsb.edu/documents/republican-candidates-debate-boulder-colorado")
debate3 <- read_html(url)
participants3 <- "BUSH|CARSON|CHRISTIE|CRUZ|HUCKABEE|KASICH|PAUL|RUBIO|TRUMP|HARWOOD|QUICK|QUINTANILLA|FIORINA"
participantsRemove3 <- "BUSH:|CARSON:|CHRISTIE:|CRUZ:|HUCKABEE:|KASICH:|PAUL:|RUBIO:|TRUMP:|HARWOOD:|QUICK:|QUINTANILLA:|FIORINA:"
```

## Sentiment Analysis
```{r, include = FALSE}
debate3DF <- scrape_transcript(debate3, participants3, participantsRemove3)
```
...
lsdDf <- dfm(debate3DF$text, dictionary = data_dictionary_LSD2015)

newLsdDf <- convert(lsdDf, to = "data.frame")

newLsdDf$doc_id <- debate3DF$speaker
...
dumbellDFLSD %>%
  ...
  geom_text(data = filter(dumbellDFLSD, speaker == "KASICH"), 
            aes(x = 1.81, y = speaker, label = "Negative"), 
...
debate3DF %>% 
  ...
  geom_col() + coord_flip() + labs(title = "Who Talked The Most?", subtitle = "Rubio used the most words", x = "speaker", y = "total words") + theme_bw()+
theme(
 # panel.grid.major=element_blank(),
  panel.grid.minor=element_blank(),
  panel.border=element_blank(),
