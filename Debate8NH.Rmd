...
url <- ("https://www.presidency.ucsb.edu/documents/republican-candidates-debate-manchester-new-hampshire-0")
debate8 <- read_html(url)
participants8 <- "BUSH|CARSON|CHRISTIE|CRUZ|KASICH|RUBIO|TRUMP|BAIER|MUIR|RADDATZ"
participantsRemove8 <- "BUSH:|CARSON:|CHRISTIE:|CRUZ:|KASICH:|RUBIO:|TRUMP:|MUIR:|RADDATZ:"
```

## Sentiment Analysis
```{r, include = FALSE}
debate8DF <- scrape_transcript(debate8, participants8, participantsRemove8)
```
...
lsdDf <- dfm(debate8DF$text, dictionary = data_dictionary_LSD2015)

newLsdDf <- convert(lsdDf, to = "data.frame")

newLsdDf$doc_id <- debate8DF$speaker
...
barDFNRC %>%
  filter(speaker != "MUIR" , speaker != "RADDATZ") %>%
...
dumbellDFLSD %>%
  ...
  geom_text(data = filter(dumbellDFLSD, speaker == "BUSH"), 
            aes(x = 2.84, y = speaker, label = "Negative"), 
            color = red, size = 2.5, fontface = "bold", vjust = -1)+
  geom_text(data = filter(dumbellDFLSD, speaker == "BUSH"), 
            aes(x = 6.22, y = speaker, label = "Positive"), 
