...
url <- ("https://www.presidency.ucsb.edu/documents/republican-candidates-debate-houston-texas")
debate10 <- read_html(url)
participants10 <- "CARSON|CRUZ|KASICH|RUBIO|TRUMP|BLITZER"
participantsRemove10 <- "CARSON:|CRUZ:|KASICH:|RUBIO:|TRUMP:|BLITZER:"
```

## Sentiment Analysis
```{r, include = FALSE}
debate10DF <- scrape_transcript(debate10, participants10, participantsRemove10)
```
...
lsdDf <- dfm(debate10DF$text, dictionary = data_dictionary_LSD2015)

newLsdDf <- convert(lsdDf, to = "data.frame")

newLsdDf$doc_id <- debate10DF$speaker
...
dumbellDFLSD %>%
  ...
  geom_text(data = filter(dumbellDFLSD, speaker == "CARSON"), 
            aes(x = 2.33, y = speaker, label = "Negative"), 
            color = red, size = 2.5, fontface = "bold", vjust = -1)+
  geom_text(data = filter(dumbellDFLSD, speaker == "CARSON"), 
            aes(x = 5.57, y = speaker, label = "Positive"), 
  ...
  geom_text(data = filter(dumbellDFLSD, speaker == "CARSON"), aes(x = 6.75, y = speaker, label = "Difference"), color = "black", size = 2.5, vjust = -.78, fontface = "bold") + labs(x = "emotive language as % of total words", y = "speaker", title = "Sentiment differences in first 2016 GOP Primary Debate (Lexicoder Sentiment Dictionary)", subtitle = "Carson exhibits the greatest preference for positive diction" ) + theme_bw()+
theme(
 # panel.grid.major=element_blank(),
  panel.grid.minor=element_blank(),
  panel.border=element_blank(),
